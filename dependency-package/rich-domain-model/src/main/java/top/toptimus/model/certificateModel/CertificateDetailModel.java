 ...
 * 凭证一览信息
 */
public class CertificateDetailModel extends BaseResultLinkInfoModel {
     ...
     */
    public CertificateDetailModel(String businessUnitId , String certificateMetaId ,String certificateTokenId ) {
    ...
    private void buildAllResultLinks() {
        ...
        resultContext.setResultUnits(new ArrayList<ResultUnit>(){{
            add(new ResultUnit(ServiceURLConstants.GET_META_URL + "?" + META_ID + "="
                    + certificateMetaId,
                    ServiceURLConstants.TARGET_BILL_BUSINESS_UNIT_ID_URL+businessUnitId +ServiceURLConstants.CERTIFICATE_TOKEN_ID_URL + certificateTokenId + ServiceURLConstants.CERTIFICATE_META_ID_URL + certificateMetaId));
        }});
    }
}
